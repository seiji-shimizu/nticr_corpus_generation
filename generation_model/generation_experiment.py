...
class T5FineTuner(pl.LightningModule):
    def __init__(self, name_space):
        super().__init__()
        for arg in vars(name_space):
            self.hparams[arg] = getattr(name_space, arg) 
        # トークナイザー
        tokenizer = T5Tokenizer.from_pretrained(self.hparams.tokenizer_name_or_path, is_fast=True)
